...
export class SimpleModal extends BaseModal {
  ...
  constructor(message: string, private onClose?: () => void) {
    ...
    background.lineStyle(3, Colors.PANEL_BORDER).beginFill(Colors.PANEL_BACK).drawRoundedRect(0, 0, WIDTH, HEIGHT, 10);
    this.addChild(background);

    this.scoreText = new PIXI.Text(message, { fontSize: 25, fontFamily: Fonts.UI, fill: 0xffffff, wordWrap: true, wordWrapWidth: WIDTH - 100 });
